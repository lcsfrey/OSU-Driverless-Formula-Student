...
def add_ResNet50_conv3_body(model):
    return add_ResNet50_body(model, (3, 3))
...
def add_ResNet50_body(model, block_counts, base_filters=256, scale_factor=2, 
                    freeze_at=None, dilations=None):
    """
    Add a ResNet body from input data up through to the residual stage equal to
    the length of block_counts
    """
    num_stages = len(block_counts)
    stages = [0] + list(range(2, num_stages+2))
    freeze_at = cfg.TRAIN.FREEZE_AT
    if freeze_at not in stages:
        logger.error("cfg.TRAIN.FREEZE_AT must be in stages\n"
                  "cfg.TRAIN.FREEZE_AT == [%s]\n"
                  "stages == %s", str(cfg.TRAIN.FREEZE_AT), 
                  ', '.join([str(s) for s in stages]))
        raise ValueError("cfg.TRAIN.FREEZE_AT must be less an available len(block_counts)")

    if cfg.RESNETS.DILATIONS is None: 
        dilations = [1] * num_stages
    else:
        dilations = cfg.RESNETS.DILATIONS
        
    assert num_stages == len(dilations)

    # add the stem (by default, conv1 and pool1 with bn; can support gn)
    s, dim_in = globals()[cfg.RESNETS.STEM_FUNC](model, 'data')
    ...
    for stage_num in stages[1:]:
        if freeze_at == stage_num:
            model.StopGradient(s, s)
            
        scale = scale_factor ** (stage_num-2)
        out_filters = in_filters * scale
        blocks_in_stage = block_counts[stage_num-2]
        bottleneck = dim_bottleneck * scale_factor ** (stage_num-2)
        logger.info("in dims %s" % str(in_filters))
        logger.info("bottleneck %s" % str(bottleneck))
        logger.info("out dims %s" % str(out_filters))
        
        s, dim_in = add_stage(
            model, 'res'+str(stage_num), s, blocks_in_stage, in_filters, 
            out_filters, bottleneck, dilations[stage_num-2]
        )
    ...
    return s, dim_in, 1. / (2**num_stages+1) * dilation_product
