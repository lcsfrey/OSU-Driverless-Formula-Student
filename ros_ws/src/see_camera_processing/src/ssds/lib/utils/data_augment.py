...
from lib.utils.visualize_utils import draw_bbox
...
class preproc(object):
    ...
    def __call__(self, image, targets=None):
        ...
        if self.writer is not None:
            ...
            self.writer.add_image('preprocess/input_image', image_show.transpose(2, 0, 1), self.epoch)

        image_t, boxes, labels = _crop(image, boxes, labels)
        if self.writer is not None:
            image_show = draw_bbox(image_t, boxes)
            self.writer.add_image('preprocess/crop_image', image_show.transpose(2, 0, 1), self.epoch)

        image_t = _distort(image_t)
        if self.writer is not None:
            image_show = draw_bbox(image_t, boxes)
            self.writer.add_image('preprocess/distort_image', image_show.transpose(2, 0, 1), self.epoch)
        ...
        #if self.writer is not None:
        #    image_show = draw_bbox(image_t, boxes)
        #    self.writer.add_image('preprocess/expand_image', image_show.transpose(2,0,1), self.epoch)

        image_t, boxes = _mirror(image_t, boxes)
        if self.writer is not None:
            image_show = draw_bbox(image_t, boxes)
            self.writer.add_image('preprocess/mirror_image', image_show.transpose(2,0,1), self.epoch)
