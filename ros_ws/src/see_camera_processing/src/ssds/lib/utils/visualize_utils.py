...
def images_to_writer(writer, images, prefix='image', names='image', epoch=0):
    ...
    for image, name in zip(images, names):
        writer.add_image('{}/{}'.format(prefix, name), image.transpose(2,0,1), epoch)
...
def viz_prior_box(writer, prior_box, image=None, epoch=0):  
    ...
    for k, f in enumerate(prior_box.feature_maps):
        ...
        writer.add_image('example_prior_boxs/feature_map_{}'.format(k), 
                         image_show.transpose(2, 0, 1), epoch)
...
def draw_detections(image, all_bbxs, 
                    colors={0:(127,127,0), 1:(255,0,0), 2:(0,0,255), 3:(0,0,0), 4:(0,255,0), 5:{255,255,255}}):
    img = image.copy()
    #bbxs = np.array(bbxs).astype(np.int32)
    for class_id, bbxs in enumerate(all_bbxs):
        for bbx in bbxs:
            score = bbx[-1]
            bbx = np.round(bbx).astype(np.int32)
            logger.debug("class {} bbox {}".format(class_id, bbx[:-1]))
            cv2.rectangle(img, (bbx[0], bbx[1]), (bbx[2], bbx[3]), colors[class_id], 5)
    return img

def viz_detections(writer, images, detections, epoch, name='train'):
    for k, (image, boxes) in enumerate(zip(images, detections)):
        print(image.shape)
        print(boxes)
        image = draw_detections(image, boxes)
        writer.add_image('detections/{}.{}'.format(name, k), image.transpose(2,0,1), epoch)
