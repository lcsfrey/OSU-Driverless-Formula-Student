...
class AttrDict(dict):
    ...
    def __str__(self):
        def attr_dict_to_str(d, indent=0):
            out_str = ''
            for k, v in d.items():
                if isinstance(v, (AttrDict, dict)):
                    out_str += str(k) + ':\n' + attr_dict_to_str(v, indent+4)
                else:
                    out_str += ' '*indent + '{}: {}\n'.format(str(k), str(v))
            return out_str
        return '\n'+attr_dict_to_str(self)
...
__C.TRAIN.CHECKPOINTS_EPOCHS = 25
...
__C.TRAIN.LR_SCHEDULER.COSINE_LAST_EPOCH = 50
# warm_up epochs
...
# Whether to use focal loss or not
__C.MATCHER.USE_FOCAL_LOSS = False
__C.MATCHER.FOCAL_LOSS_GAMMA = 2
...
# test image size
__C.DATASET.TEST_IMAGE_SIZE = __C.TEST.IMAGE_SIZE
# image mean
...
def _check_and_coerce_cfg_value_type(value_a, value_b, key, full_key):
    ...
    if isinstance(value_b, np.ndarray):
        ...
        value_a = str(value_a)
    elif isinstance(value_b, unicode):
        value_a = unicode(value_a)
