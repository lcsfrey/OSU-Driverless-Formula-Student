...
class FSSD(nn.Module):
    ...
    def __init__(self, base, extras, head, features, feature_layer, num_classes):
        ...
        logger.debug("Num extras: {}".format(len(self.extras)))
        logger.debug("Extras: {}".format(self.extras))

        logger.debug("Num pyramid_transforms: {}".format(len(self.transforms)))
        logger.debug("Transforms: {}".format(self.transforms))

        logger.debug("Num pyramid layers: {}".format(len(self.pyramids)))
        logger.debug("Pyramids: {}".format(self.pyramids))

        # logger.debug(self.base)
    ...
    def forward(self, x, phase='eval'):
        ...
        for k in range(len(self.base)):
            #logger.debug('layer {}: {}'.format(k, self.base[k]))
            #logger.debug('input {}'.format(x.size()))
            x = self.base[k](x)
...
class BasicConv(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1,
                 groups=1, relu=True, bn=False, bias=True):
...
def add_extras(base, feature_layer, mbox, num_classes, version):
    ...
    for layer, depth in zip(feature_layer[0][0], feature_layer[0][1]):
        logger.debug("adding extra layer: {} {}".format(layer, depth))
        if 'lite' in version:
            if layer == 'S':
    ...
    logger.debug("Done adding extras. Total extras: {}".format(len(extra_layers)))
