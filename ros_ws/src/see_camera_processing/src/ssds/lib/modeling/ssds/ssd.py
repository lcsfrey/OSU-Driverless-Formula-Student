...
class SSD(nn.Module):
    ...
    def __init__(self, base, extras, head, feature_layer, num_classes, lite=False):
...
def add_extras(base, feature_layer, mbox, num_classes, version):
    ...
    for layer, depth, box in zip(feature_layer[0], feature_layer[1], mbox):
        if 'lite' in version:
            if layer == 'S':
                extra_layers += [
                    _conv_dw(in_channels, depth, stride=2,
                             padding=1, expand_ratio=1)
                ]
            elif layer == '':
                extra_layers += [
                    _conv_dw(in_channels, depth, stride=1,
                             padding=0, expand_ratio=1)
                ]
        else:
            if layer == 'S':
                extra_layers += [
...
def _conv_dw(inp, oup, stride=1, padding=0, expand_ratio=1):
    return nn.Sequential(
        ...
        nn.Conv2d(oup * expand_ratio, oup * expand_ratio, 3, stride, padding,
                  groups=oup * expand_ratio, bias=False),
        nn.BatchNorm2d(oup * expand_ratio),
        nn.ReLU6(inplace=True),
        # pw-linear
...
def build_ssd(base, feature_layer, mbox, num_classes):
    ...
    return SSD(base_, extras_, head_, feature_layer, num_classes, lite=False)
...
def build_ssd_lite(base, feature_layer, mbox, num_classes):
    ...
    return SSD(base_, extras_, head_, feature_layer, num_classes, lite=True)
