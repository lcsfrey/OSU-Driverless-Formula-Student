...
class Solver(object):
    ...
    def train_model(self):
        ...
        for epoch in iter(range(start_epoch+1, self.max_epochs+1)):
            ...
            if 'eval' in cfg.PHASE:
                prec, rec, ap = self.eval_epoch(self.model, self.eval_loader, self.detector, self.criterion, self.writer, epoch, self.use_gpu)
                if ap > best_ap:
                    best_ap = ap
                    best_model_state_dict = self.model.state_dict()
    ...
    def validate_model(self):
        ...
        num_folds = len(cfg.DATASET.TRAIN_SETS)
        eval_stats = np.zeros(3)
        print(eval_stats)

        for val_set_index in range(num_folds):
            print("FOLD", val_set_index)
            self.model.load_state_dict(clean_state_dict)

            fold_cfg = copy.deepcopy(self.cfg)
            fold_cfg.DATASET.TRAIN_SETS = [
                s for i, s in enumerate(self.cfg.DATASET.TRAIN_SETS)
                if i != val_set_index
    	    ]
            fold_cfg.DATASET.TEST_SETS = [
                s for i, s in enumerate(self.cfg.DATASET.TRAIN_SETS)
                if i == val_set_index
    	    ]
            self.init_data_loaders(fold_cfg)

            fold_cfg.EXP_DIR = os.path.join(fold_cfg.EXP_DIR, 'fold{}'.format(val_set_index))
            fold_cfg.LOG_DIR = os.path.join(fold_cfg.LOG_DIR, 'fold{}'.format(val_set_index))

            print("Using ", fold_cfg.DATASET.TEST_SETS, "as validation set")

            self.train_model()
            prec, rec, ap = self.eval_epoch(self.model, self.eval_loader, self.detector, self.criterion, self.writer, epoch, self.use_gpu)
            print("FOLD FINISHED")
            print("prec: {} rec: {} ap: {}".format(prec, rec, ap))
            eval_stats = eval_stats + np.array([prec, rec, ap])

        eval_stats /= num_folds
        print("KFOLD VALIDATION FINISHED")
        print("FINAL STATS")
        print("prec: {} rec: {} ap: {}".format(eval_stats[0], eval_stats[1], eval_stats[2]))
    ...
    def train_epoch(self, model, data_loader, optimizer, criterion, writer, epoch, use_gpu):
        ...
        for i in range(epoch_size):
            ...
            # log per iter
            #log = '\r==>Train: || {iters:d}/{epoch_size:d} in {toc_time:.3f}s [progress] || loc_loss: {loc_loss:.4f} cls_loss: {cls_loss:.4f}\r'.format(
            #        progress='#'*int(round(10*i/epoch_size)) + '-'*int(round(10*(1-i/epoch_size))), iters=i, epoch_size=epoch_size,
    ...
    def visualize_epoch(self, model, data_loader, priorbox, writer, epoch, use_gpu):
        ...
        images = torch.tensor(preproc(image, anno)[0].unsqueeze(0))
        if use_gpu:
            images = images.cuda()
